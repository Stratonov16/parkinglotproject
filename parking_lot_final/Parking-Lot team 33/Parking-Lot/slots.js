      ...
      if (data[index - 1].status == 1) {
        ...
        for (let i = 0; i < commas.length; i++) {
          ...
          if (
            (h1 < h1curr && h2 <= h1curr) ||
            (h1 >= h2curr &&
              ((h2 > h2curr && h2 > h1curr) || (h2 < h2curr && h2 < h1curr)))
          ) {
            console.log("hre" + " " + i);
            continue;
            //document.getElementById(id).style.backgroundColor = "green";
...
slots();
let selection;
document.getElementById("proceed_slots").onclick = function getSelection() {
  console.log("here");
  for (let [index, id] of map1) {
  ...
  let booking_time = h1.toString() + ":" + m1 + "-" + h2.toString() + ":" + m2;
  localStorage.setItem("booking_time", booking_time);
  localStorage.setItem("old_bookings", data[selection - 1].time);
  let proceed = document.getElementById("proceed_slots");
  console.log("hi");
  window.location = "ameneties.html";
